...
module.exports = {
    plugin: function (schema, documents, config) {
        ...
        if (config && config.typesFile) {
            additionalImports += "import * as t from " + JSON.stringify(config.typesFile);
            tPrefix = "t.";
        }
        ...
        documents.forEach(function (d) {
            return d.content.definitions.forEach(function (node) {
                if (graphql_1.isExecutableDefinitionNode(node) &&
        ...
        documentOperations.forEach(function (o) {
            var typeVal = "" + tPrefix + plugin_helpers_1.toPascalCase(o.name + "_" + o.op);
            MockOperationTypes.push(o.name + ": ErrorOrValue<" + typeVal + " | PartialDeep<" + typeVal + ">>");
        });
        var MockBaseTypesBody = Object.keys(allTypes)
            ...
            else if (graphql_1.isInterfaceType(type) || graphql_1.isUnionType(type)) {
                typeVal = schema
                    .getPossibleTypes(type)
                    .map(function (t) { return "TypeMock<" + tPrefix + t.name + ">"; })
                    .join(" | ");
            }
